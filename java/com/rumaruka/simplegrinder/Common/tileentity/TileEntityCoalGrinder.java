...
public class TileEntityCoalGrinder  extends TileEntityLockable implements ITickable, ISidedInventory
{
	 private static final int[] SLOTS_TOP = new int[] {0};
	    ...
	    {
	        boolean flag = stack != null && stack.isItemEqual(this.grindItemStacks[index]) && ItemStack.areItemStackTagsEqual(stack, this.grindItemStacks[index]);
	                ...
	                if (!this.isBurning() && this.canSmelt())
	                {
	                    ...
	                    if (this.isBurning())
	                    {
	                        ...
	                        if (this.grindItemStacks[1] != null)
	                        {
	                            ...
	                            {
	                                this.grindItemStacks[1] = grindItemStacks[1].getItem().getContainerItem(grindItemStacks[1]);
	                            }
	                        }
	                    }
	                }
	    ...
	    private boolean canSmelt()
	    {
	        ...
	        else
	        {
	            ...
	            if (this.grindItemStacks[2] == null) return true;
	            if (!this.grindItemStacks[2].isItemEqual(itemstack)) return false;
	            int result = grindItemStacks[2].stackSize + itemstack.stackSize;
	            return result <= getInventoryStackLimit() && result <= this.grindItemStacks[2].getMaxStackSize(); //Forge BugFix: Make it respect stack sizes properly.
	        }
	    }
	    ...
	    public void smeltItem()
	    {
	        if (this.canSmelt())
	        {
	            ...
	            {
	                this.grindItemStacks[2].stackSize += itemstack.stackSize; // Forge BugFix: Results may have multiple items
	            }

	            if (this.grindItemStacks[0].getItem() == Item.getItemFromBlock(Blocks.SPONGE) && this.grindItemStacks[0].getMetadata() == 1 && this.grindItemStacks[1] != null && this.grindItemStacks[1].getItem() == Items.BUCKET)
	            {
